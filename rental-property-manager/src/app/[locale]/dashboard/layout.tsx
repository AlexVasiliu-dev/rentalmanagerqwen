...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const navigation = [
    ...
    { name: t('leases'), href: "/dashboard/leases", icon: FileText, roles: ["ADMIN", "MANAGER"] },
    { name: t('meterReadings'), href: "/dashboard/meter-readings", icon: FileText, roles: ["RENTER", "MANAGER"] },
    { name: "Facturi", href: "/dashboard/bills", icon: DollarSign, roles: ["RENTER", "MANAGER", "ADMIN"] },
            ...
            <div className="flex items-center gap-4">
              <div className="text-sm text-gray-600 hidden sm:block">
                ...
                <span className="ml-2 px-2 py-1 bg-blue-100 text-blue-800 rounded text-xs">
                  {session?.user.role === "ADMIN" ? "Proprietar" : session?.user.role === "MANAGER" ? "Manager" : "Chiriaș"}
                </span>
              </div>
